...
export default function VerifierPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!file) return;
    try { const r = await verify.mutateAsync(file); (r.final_result === 'VALID' ? toast.success : toast.error)(r.final_result, r.headline); }
    catch (err) { toast.error('Verification failed', (err as ApiError).message); }
  };
  const r = verify.data;
